...
export default function Experience({ data }: { data?: any }) {
  ...
  const content = data?.[lang] || EXPERIENCE_CONTENT[lang];
  const bgImg = data?.image_url || "/images/experience-1.jpg";
  ...
  return (
    <section id="experience" className="relative flex w-full items-center bg-cream overflow-hidden py-24 lg:py-32">
      ...
      {/* Background Gambar Sebelah Kiri dengan Gradien Masking */}
      <motion.div 
        variants={imageReveal} 
        initial="hidden" 
        whileInView="visible" 
        viewport={{ once: true, margin: "-100px" }} 
        className="absolute inset-0 z-0 flex justify-start pointer-events-none"
      >
        <div className="relative w-full h-full lg:w-[70%]">
          <Image 
            src={bgImg} 
            alt="Pengalaman di Villa Omahku" 
            fill 
      ...
      <div className="container mx-auto px-6 pr-8 md:pr-20 lg:pr-32 relative z-10 flex justify-end">
        <motion.div 
          ...
          whileInView="visible" 
          viewport={{ once: true, margin: "-100px" }} 
          className="w-full lg:w-[60%] xl:w-[50%] flex flex-col items-start text-left mt-64 lg:mt-0"
        >
